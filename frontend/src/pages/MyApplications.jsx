      ...
      <div className="max-w-4xl mx-auto card-glass p-6">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {applications.map((a) => (
              <div key={a._id} className="card-soft p-4">
                <h3 className="font-semibold">{a.job?.title || "(no title)"}</h3>
                <p className="text-sm text-slate-300">{a.job?.companyName}</p>
                ...
                {a.interviewLink && (
                  <a href={a.interviewLink} target="_blank" rel="noreferrer" className="text-cyan-300 text-sm">Join Interview</a>
                )}
                {a.resume && (
                  <a href={`http://localhost:5001/${a.resume}`} target="_blank" rel="noreferrer" className="text-cyan-300 text-sm">View Resume</a>
                )}
              </div>
            ))}
          </div>
